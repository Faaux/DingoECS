...
namespace decs
{
	DiWorld::DiWorld(uint32_t chunkSize)
		: _chunkSize(chunkSize), _idCounters(chunkSize), _componentStorage(chunkSize)
	{
	...
	void DiWorld::Refresh()
	{
		...
		for (auto &id : _addedEntites)
		{
			...
			for (auto system : _systems)
			{
				...
				{
					system->AddEntity(DiEntityId(id._id));
				}
			}
		}
		_addedEntites.clear();
